...
class _Page22PageState extends State<Page22> {
  ...
  int? selectYear;
  List<int> listYear = [];
  List<ngaydangkiem> listDate = [];
  ngaydangkiem? valueDate;
  ...
  getDate(String date) async {
    String url =
        'https://api.dantimegkiem.online/api/Slot/slot_home?datetime=$date';
        // 'https://api.dangkiem.online/api/Slot?datetime=$date&stationId=$id';
    var response = await http.get(Uri.parse(url));
    if (response.statusCode == 200) {
      var jsonResponse = jsonDecode(response.body);
      setState(() {
        listDate = List<ngaydangkiem>.from(
            jsonResponse['data'].map((date) => ngaydangkiem.fromJson(date)));
      });
    }
  }
                ...
                Row(
                    ...
                    children: <Widget>[
                      ...
                      Container(
                        ...
                        width: 160,
                        height: 60,
                        child: DropdownButton<ngaydangkiem>(
                            hint: const Text(
                              " Chọn giờ đến",
                              textAlign: TextAlign.left,
                              style: TextStyle(fontSize: 17),
                            ...
                            iconSize: 28,
                            isExpanded: true,
                            items: listDate.map((ngaydangkiem item) {
                              return DropdownMenuItem<ngaydangkiem>(
                                child: Text(
                                  item.data
                                      // .toString()
                                  ,
